...
class SignInOptions(BaseModel):
    ...
    emailAndPassword: bool = Field(default=False, description="Enable email and password sign-in")
...
class FirebaseExtensionConfig(BaseModel):
    ...
    signInSuccessUrl: str = Field(default="/", description="The URL to redirect to after a successful sign-in")
    tosLink: Optional[str] = Field(None, description="Link to the terms of service")
    privacyPolicyLink: Optional[str] = Field(None, description="Link to the privacy policy")
